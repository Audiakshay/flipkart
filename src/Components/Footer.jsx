...
function Footer() {
  ...
  return (
    <div className="w-full bg-gray-900">
        <div className='flex mx-12 py-10 pb-10'>{
        footerList.map((item) => (
            <FooterLists key={item.title} title={item.title} list={item.list}/>       
        ))
    }</div>
    <div className='flex items-center justify-evenly h-20 border-t  border-indigo-300'>
        ...
        <p className='text-white font-semibold text-sm'>&copy; 2007-2023 Flipkart.com</p>
